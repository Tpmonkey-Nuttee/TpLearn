...
class Updater(Cog):
    ...
    async def update(self) -> None:
        ...
        for gid in d:
            ...
            messages = await self.get_messages(d[gid]['active'])

            try:
                ret = await self.update_active(messages, works, d[gid])
            except:
                await self.bot.log(
                    __name__, 
                    f":negative_squared_cross_mark: Unable to update works at {gid} with error: {traceback.format_exc()}"
                )
            else:
                if len(ret) != 2:
                    await self.bot.log(
    ...
    async def update_active(self, messages: list, works: list, channels: dict, bypass: bool = False) -> list:  
        ...
        # To use for logging.
        messages_output = ["Total Works {}".format(cw), "Total Messages {}".format(cm)]
    ...
    async def _update(self, ctx: Context, gid: str, bypass: bool = False) -> None:
        ...
        try:
            result = await self.update_active(messages, works, d[gid], True)
        except:
